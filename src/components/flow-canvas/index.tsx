	...
	const handleSendBranchMessage = useCallback(
		...
		try {
			if (isMultiModel && branchAIs.length > 1) {
				// 🔥 MOCK MODE: Generate mock responses for multi-model mode
				for (const ai of branchAIs) {
					const modelName = resolveModel(ai.id)
					console.log(`🔍 [MOCK MODE] Branch multi-model: Generating mock response for ${ai.name}`)
				...
				const modelName = resolveModel(ai.id)
				console.log(`🔍 [MOCK MODE] Branch single mode: Generating mock response for ${ai?.name}`)
